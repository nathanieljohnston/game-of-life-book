...
\begin{document}
	\begin{tikzpicture}%
		...
		\node[inner sep=0pt,anchor=north west] (ca) at (0,9.05) {\includegraphics[width=3.5cm]{0e0p_timeline_17179869184.png}};
		...
		% ZOOM MIDDLE
			...
			\node[inner sep=0pt,anchor=north west] (zoom0) at (\bglx,\bgty) {\includegraphics[width=4.5cm]{0e0p_timeline_17179869184_zoom0.png}};
		...
		% ZOOM RIGHT
			...
			\node[inner sep=0pt,anchor=north west] (zoom1) at (\bglx,\bgty) {\includegraphics[width=4.5cm]{0e0p_timeline_17179869184_zoom1.png}};
			
			\draw[gridgray,line width=0.6pt] (\bglx,\bgty) rectangle (\bgrx,\bgby);
		% END ZOOM RIGHT
		
		\node[inner sep=0pt,anchor=north west] (genarr) at (9.93,8) {\color{black}{$\xrightarrow{\text{\clock{4}{5} $2^{29}$}}$}};
		
		% GLIDER PATH ARROWS
		\draw[color=gray,-stealth] (14.5,7.5) -- (14,7);
		\draw[color=gray,-stealth] (7.99,7.5) -- (7.49,7);
		
		\draw[gray] (2.35,5.60) node {\small\texttt{S}};
		\draw[gray] (0,10) node[anchor=north west,align=left] {\texttt{t = 17}\ \texttt{179}\ \texttt{869}\ \texttt{184}\\ \texttt{{} \ = 32 × 2$^{\texttt{29}}$ = 2$^{\texttt{34}}$}};
	\end{tikzpicture}
\end{document}