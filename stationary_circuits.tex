...
As another application of still life factories, we now construct a stable Heisenburp---a stable pattern that can detect the presence of a spaceship without affecting it at all, even temporarily (recall that we saw some \emph{periodic} Heisenburps in Section~\ref{sec:p46_heisenburps}). No such pattern can be constructed to detect the presence of a glider, since gliders do not emit any sparks. However, xWSSes are plenty sparky, and we can use factories to construct stable Heisenburps that detect their presence. In particular, we now construct a stable Heisenburp for an MWSS.

Much like our highway robber, our starting point is a reaction in which an MWSS triggers a still life to explode into a chaotic mess and/or a glider. One such reaction is displayed in Figure~??...

% MWSS is nice because sparky but easier to construct than HWSS. strikes a nice balance between LWSS and HWSS.

% Exercise: make stable HWSS heisenburp
...
% Silver reflector is already in chapter 3!
...
\vspace*{-0.4cm}\hrulefill\vspace*{-0.3cm}\footnotesize\begin{multicols}{2}\vspace*{-0.4cm}\raggedcolumns\interlinepenalty=10000
	...
	\begin{problem}\label{exer:other_toolkit_stable_conduits}
		Another toolkit for duplicating and synchronizing gliders in stable circuitry, which can be used instead of Table~\ref{tab:conduit_phase_changers}, is provided at \httpsurl{conwaylife.com/forums/viewtopic.php?f=&p=51934}. \smallskip
...
\httpsurl{conwaylife.com/forums/viewtopic.php?f=&p=51934}
...
%	
%	
%	\begin{problem}\label{exer:factory_toggle}
%		A factory that cleanly destroys its output object if a second input signal is received is called a \emph{toggle factory},\index{toggle!factory} and one is displayed below.
%		
%		% FIGURE
%		% TODO: WHAT EXERCISE TO MAKE HERE?
%	\end{problem}
