...
% TRIGGER1 just triggers. TRIGGER2 also does self-destruction stuff
% STuff from here on can be simplified considerably
...
\begin{itemize}
	...
	\item[2)] a trigger glider for the one-glider seed, starting the double wickstretcher on its way at a speed of c/4 diagonally;\smallskip% combine 1) and 2)? We already saw them
...
% FIGURE. Probably a lot earlier, actually.
% minimal Speed Demonoid here. 3285622c/16493928
...
%\begin{theorem}[Speed Demonoids Attain Any Speed]\label{thm:speed_demonoid_all_speeds}
%	Suppose $p$ and $q$ are positive integers with $p/q < 1/4$. Then there exist positive integers $m$ and $n$ such that
%	\[
%		\frac{p}{q} = \frac{3285622 + 2n}{16493928 + 8n + 8m}.
%	\]
%	In particular, this means that the Speed Demonoid from Figure~?? can be adjusted to have any rational speed strictly slower than $c/4$.
%\end{theorem}
%
%\begin{proof}
%	Multiplying both sides of this equation by the denominators and then rearranging shows that it is equivalent to
%	\[
%		m(4p) - n(q - 4p) = 1642811q - 2061741(4p).
%	\]
%	If we make the change of variables $a = 4p$ and $b = q - 4p$ (noting that $p/q < 1/4$ implies $q - 4p > 0$, so $a$ and $b$ are both positive integers), then this equation simplifies further to
%	\[
%		ma - nb = 1642811b - 418930a.
%	\]
%	Since $1642811b - 418930a$ is a multiple of $\mathrm{gcd}(a,b)$, B\'ezout's identity (Theorem~\ref{thm:linear_diophantine}) immediately tell us that there are positive integers $m$ and $n$ satisfying this equation.
%\end{proof}
% Exercise: Given rational speed, find m and n
% Adjustment script is here: https://www.conwaylife.com/forums/viewtopic.php?f=2&t=4711&start=25#p105167 (v4)
