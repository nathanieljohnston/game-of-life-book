...
\begin{itemize}
	...
	\item In most of these glider sequences (including the very first one, from Figure~\ref{fig:lane_8_0degree}), there are numerous gaps of 90 generations. The reason for this is that we want to be able to feed these single-channel glider recipes through standard components like Snarks and the easy-to-synthesize Lx200-assisted\index{Lx200} syringe\index{syringe} from Exercise~\ref{exer:syringe_Lx200} (which has a repeat time of~$90$ generations). Most of the gliders that have a timing of 90 generations could actually be moved much closer (i.e., the previous glider's collision settles down before they arrive) if we so desired. The numerous gliders with a timing of 91 generations are similar, except their timing matters mod~$2$ due to p$2$ components like blinkers in an intermediate reaction, so they cannot arrive after exactly 90 generations.
	
	% Talk about how 115 gen is universal, and larger is conjectured (larger spacing is better)
...
% talk about block move recipe here, keep the not exhausive bullet point last so it leads in
