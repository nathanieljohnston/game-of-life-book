...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{A c/4 Elbow-Carrying Spaceship}\label{sec:fast_demonoid_elbow_carry}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[!htb]
	\centering
	\embedlink{boatstretcher_seed}{\vcenteredhbox{\patternimg{0.122}{boatstretcher_seed_0}} \vcenteredhbox{\genarrow{553}} \vcenteredhbox{\patternimg{0.122}{boatstretcher_seed_553}}}
...
\begin{figure}[!htb]
	\centering
	\embedlink{boatstretcher_shoot_down}{\vcenteredhbox{\patternimg{0.15}{boatstretcher_shoot_down_0}} \vcenteredhbox{\genarrow{252}} \vcenteredhbox{\patternimg{0.15}{boatstretcher_shoot_down_252}} \vcenteredhbox{\genarrow{187}} \vcenteredhbox{\patternimg{0.15}{boatstretcher_shoot_down_439}}}
	
	\caption{By colliding a glider with a boat near the end of the boat wick, we can ignite a $c/3$ fuse. That $c/3$ fuse eventually catches up with the crab and glider at the front, causing them to self-destruct back into some simple ash objects (which could then be converted back into an elbow block by subsequent gliders).}\label{fig:boatstretcher_shoot_down}
\end{figure}
...
% Exercise with another way to destroy snark: just fire lots of gliders from the forward direction (this is different from the alread-existing exercise about pre-constructing the temporary snark)

% FIGURE
% Rotate so that it moves to the top-right instead of the top-left
...
% Speed Demonoid construction script: https://conwaylife.com/forums/viewtopic.php?p=105167#p108786
...
As usual, the main body of the Speed Demonoid is its single-channel recipe -- a long stream of gliders that encodes all the construction work that will have to be done to support the creation of a meandering path ahead of the spaceship, of exactly the right length to produce the required speed.

First the single-channel recipe hits a Scorbie Splitter, starts up its construction arm, and builds

1) a complete 1-glider seed for a double crab-wickstretcher, pointing toward the locations where the next Snark and Scorbie Splitter will have to be built;
2) a trigger glider for the 1G seed, starting the double wickstretcher on its way at a speed of c/4 diagonally;
3) a few bait still lifes at the right locations to allow each of the two wicks to be activated, with a particular type of clean fuse that burns slowly, at c/3 instead of the usual 2c/3 or lightspeed;
4) a series of unidirectional gliders in two slow salvos, traveling alongside the wick without disturbing it, which will eventually construct the new copies of the required Snark and Scorbie Splitter ahead of the spaceship on that side.
...
TRIGGER2 activates the second wick, which also burns until it reaches the leading crab spaceship. But this time, the "guard glider" on that side of the wickstretcher is placed so that the burning wick reaction reaches forward and destroys the crab, stopping it in its tracks and making a small amount of debris, which can be used by the following slow salvo to build a new Scorbie Splitter on that side.
...
The adjustment math for these Speed Demonoids is surprisingly easy.
...
You can't move the trigger gliders any closer than they are in a 3285622c/16493928 Speed Demonoid, because (for TRIGGER1) the attempt to light the fuse will fail because some of the mechanism is still under construction, or (TRIGGER2) the single-channel salvo will start to run into the next Snark while it's still under construction.
...
Adjusting the trigger gliders is more interesting. It turns out that delaying the TRIGGER1 glider by one tick, will change the location where the burning wick will catch up with the crab spaceship by exactly four ticks -- one cell diagonally. The Snark will then be built at a slightly different location, off by (-1,-1) compared to the unadjusted TRIGGER1.
...
% Script is here: https://www.conwaylife.com/forums/viewtopic.php?f=15&t=4117#p105196
...
\vspace*{-0.4cm}\hrulefill\vspace*{-0.3cm}\footnotesize\begin{multicols}{2}\vspace*{-0.4cm}\raggedcolumns\interlinepenalty=10000
	...
	\begin{problem}\label{exer:middling_demonoid_faster}
		By separating the Cordership-creation and Cordership-destruction portions of the single-channel glider recipe, make a version of the ``middling'' Demonoid from Figure~\ref{fig:c256_demonoid} that advances by at least $2^{15}$ cells per period, instead of just $2^{14}$. What is the speed of this new, faster Demonoid?
