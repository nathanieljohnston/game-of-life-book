...
\begin{itemize}
	...
	\item In December 2018, Chris Cain completed a \emph{camelship}\footnote{See \httpsurl{conwaylife.com/wiki/Camelship}.} by modifying an earlier \emph{volatility~1 oscillator}\footnote{See \httpsurl{conwaylife.com/wiki/Volatility}.} design. The oscillator ensures that all its cells oscillate at the pattern's full period, by using single-channel glider recipes to destroy then rebuild its own circuitry. The camelship consists of a very similar single-channel signal loop, but the circuit reconstruction happens at a (3,1) offset.
\end{itemize}
...
% Putting figure down here to force exercises header onto same page as it
\begin{figure}[!htb]
	\centering
