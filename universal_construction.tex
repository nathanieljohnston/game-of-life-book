...
% Give big picture: We show how to construct spaceships with any slope first (but not any speed), then show how to construct diagonal spaceships with any speed (but not any slope)
...
In addition to changing Gemini's slope, we can also use this method of inserting additional \texttt{INC} operations to increase its speed. For example, if we insert another $3072$ \texttt{INC} operations on the Gemini's northwest construction elbow's path,\footnote{This is the exact same number of additional \texttt{INC} operations that we added in the knightship Geminoid.} and another $2048$ \texttt{INC} operations on its northeast construction elbow's path, we get a Geminoid that travels exactly twice as far per period: $(2048,10240)$. The speed of this new Geminoid depends a bit on some fine details like how much space we leave between the new \texttt{INC}-triggering gliders, and how tightly we push the ends of the Geminoid together. If we optimize this Geminoid in both of these regards, its speed will be $(2048,10240)c/35461914 = (1024,5120)c/17730957$, which is roughly twice as fast as the original Gemini.
% TODO: Double check the above speed. I used 575 in the calc, should it be 579?

By adding more and more \texttt{INC} operations to each of the construction elbows, we can construct Geminoids that travel faster and faster. However, this speed increase is limited by the speed at which we can perform \texttt{INC} operations. Since the Gemini's circuitry contains a Callahan G-to-H\index{Callahan G-to-H} (refer back to Figure~??), which has a repeat time of $575$ generations, any \texttt{INC}-triggering gliders that we add to the Gemini must be separated by at least that much. Furthermore, since each \texttt{INC} operation pushes the elbow $1$ full diagonal (i.e., $4$ generations) farther away, it takes $575+4 = 579$~generations to...

% lead into asymptotic speed limit
% When talking about lowering past 579 gens, mention lots of crossings to be careful of
% How does inserting an additional glider for an additional INC operation in the gemini's path affect it's speed limit? this isn't discussed anywhere yet, I don't think.

% DAVE SAYS: Meaning a slope of 3 in a 45-degree coordinate system? Can't we just call this a slope of 2? You should be able to produce a true (2x, x) Gemini knightship just by adding 3072 INC instructions to the westward construction arm, lengthening the distance between the two ends appropriately, and recompiling to avoid collisions in the glider-stream crossings in the upper part of the westward arm in the southeast replicator... I haven't looked at Andrew's code closely enough yet to find out if the (1024, 5120) offset is deeply buried in underlying assumptions in the collision-avoidance code, or if it's easy to change, or if anything will even need changing...!
...
\vspace*{-0.4cm}\hrulefill\vspace*{-0.3cm}\footnotesize\begin{multicols}{2}\vspace*{-0.4cm}\raggedcolumns\interlinepenalty=10000
	...
	\begin{problemstar}\label{exer:gemini_slope_n_fixed}
		The slope-$2$ Geminoid from Figure~\ref{fig:gemini_knightship} was created by just adding additional \texttt{INC} operations to the northwest construction elbow (i.e., we increased the value that we called ``$m$'' in that section, while leaving ``$n$'' fixed at $n = 2{\thousep}048$). What Geminoid slopes are attainable by making this type of change to Gemini? That is, what slopes can be attained by increasing $m$, but not changing $n$?
	\end{problemstar}
