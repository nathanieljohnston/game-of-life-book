...
\begin{itemize}
	...
	\item Finally, we can use the elbow-moving recipes of Table~\ref{fig:90_degree_block_move} adjust the relative positioning of the three different elbows that are used throughout all of these operations.\smallskip
\end{itemize}

After we put all of this together, we get a single-channel glider recipe containing a grand total of 2,427 gliders that creates a Snark, and also moves the elbow to the far side of the Snark so that subsequent glider recipes on that lane hit the elbow after passing through the Snark. We do not provide a figure containing the Snarkmaker (it as, after all, just a single-file straight line consisting of 2,427 gliders), but the complete sequence of glider timings that make it up is provided in Appendix~\ref{sec:appendix_snarkmaker}.
...
The Snarkbreaker is \emph{considerably} simpler than the Snarkmaker.\footnote{In Life, just like in life, it is typically much easier to break something than it is to make it.}
