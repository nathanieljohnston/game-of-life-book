...
Perhaps the simplest elbow-moving spaceships to implement this idea with are Corderships, thanks to how straightforward they are to synthesize (compared to most other spaceships, anyway).

% In notes and history: Mention at-most c/5 demonoid that was constructed via this same method

% Speed limit of c/12. Introduce cordership seed stuff here.
...
% Talk about how the Cordership lets us avoid the mild slowdown caused by sending a glider back for cleanup -- don't need to send glider back from Cordership, but just back from where the Cordership was fired(?)
