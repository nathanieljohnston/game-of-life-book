...
% What \emph{is} a computer, and what kind of computer might we want to build?
...
%When a computer program like this is being executed, the computer has to keep track of where it is in the program, generally by maintaining a pointer to the line of code that will be executed next.  

Our next goal is to simplify and standardize our pseudocode so as to make it simpler to implement as a Life pattern. With this in mind, a \emph{finite-state machine}\index{finite-state machine} is a model of computation in which the computer can be in one of a finite number of states at any given time, and its state can change based on inputs that it receives. We can implement a finite-state machine with $n$ possible states via a Life pattern that has a single glider traversing a set of $n$ parallel lanes connected by other circuitry.  At any given computer clock tick, the glider will be on exactly one of the ``state'' lanes. It will then run through structures that send gliders to various inputs of various register circuits, and also send a glider to the lane representing the next state in the program.
...
Our first step toward building such a computer is to write APGsembly code that implements the desired computational task. The code in APGsembly~\ref{alg:apgsembly_add_same_register} does the job nicely---it works by repeatedly decreasing the value of one register until it reaches a value of \texttt{0}, while simultaneously increasing the value of the other register every time. The only pieces of this APGsembly that we have not yet seen are the introductory ``\texttt{\#COMPONENTS}'' and ``\texttt{\#REGISTERS}'' lines. These lines are used to tell the APGsembly compiler which registers are used in the code,\footnote{``\texttt{U0-1}'' means that the code uses registers \texttt{U0} and \texttt{U1}. The ``\texttt{-}'' indicates a range (so ``\texttt{U0-2}'' would refer to registers \texttt{U0}, \texttt{U1}, and \texttt{U2}, for example).} and what the initial values of the registers should be, respectively.\footnote{If a register is not initialized via the \texttt{\#REGISTERS} line, it starts with a value of \texttt{0}.}
...
One of the unfortunate features of sliding block registers is that the only actions they have available to them are addition and subtraction by \texttt{1}, so arithmetic operations involving large numbers stored in this registers take a long time to complete. For example, the addition code of APGsembly~\ref{alg:apgsembly_add_same_register} takes \texttt{U0+1} clock ticks to add the value of \texttt{U0} to \texttt{U1}, and the multiplication code of APGsembly~\ref{apg:mult_unary_reg} takes roughly \texttt{2 * U0 * U1} clock ticks to multiply \texttt{U0} by \texttt{U1}.\footnote{In fact, even the addition and subtraction by \texttt{1} operations take longer to complete the larger the value of the register is, since the sliding block takes longer to interact with when it is farther away.}

While larger inputs leading to longer times to perform arithmetic operations is inevitable, we can save a lot of time by representing non-negative integers in a more efficient way than just as the position of a sliding block. A sliding block register can be thought of as non-negative integers in \emph{unary}\index{unary} (i.e., the base-$1$ numeral system, in which each number is represented by tally marks or a distance from a fixed point), the register that we now introduce instead encodes non-negative integers in \emph{binary}\index{binary} (i.e., the base-$2$ numeral system).\footnote{Other bases larger than $2$ would also be quicker to work with than unary, but they would be even more complicated than binary to implement.} While performing operations on such a register is somewhat more complicated, it has the advantage of being much quicker because, for example, the non-negative integer $n$ can be stored in $O(\log(n))$ space instead of $O(n)$ space.

We call a register that stores a non-negative integer in this way a \emph{binary register},\index{binary register} and the one that we make use of is displayed in Figure~\ref{fig:binary_register}. It uses block-moving shotguns similar to the ones in sliding block registers, except that they move a block 16 cells diagonally at a time instead of just one cell diagonally. This wider spacing leaves room for additional reactions to place objects on one side of the sliding block. Every 16 cells along the sliding block's path is a designated bit location, which can contain either an empty space or a single boat in one of two slightly different positions that correspond to bits \texttt{0} and \texttt{1}.

% FIGURE

The five actions available to this binary register are a bit more abstract than the actions of the sliding block register, since they work on the individual bits of the register rather than the value that those bits represent. An \texttt{INC} action moves the pointer block (called the \emph{read head}\index{read head} of the register) to the next bit location, one step farther away. A \texttt{DEC} action moves the pointer block to the previous bit location (or keeps it in the same place if it's already at the zero position). A \texttt{READ} action removes the bit from the current pointer location, returning either a \texttt{Z} or \texttt{NZ} signal as output depending on the bit. A \texttt{SET} action places a ``\texttt{1}'' bit at the current pointer location.

Unlike a simple sliding block register where there's no way to damage the mechanism by sending action signals to it, there are definitely ways to program a binary register that will cause it to explode catastrophically. In particular, if you send a \texttt{SET} action while there's already a \texttt{1} stored at the current pointer location, it will cause irrecoverable damage. There's no built-in safety mechanism to prevent this, but these problems can easily be avoided by always sending a \texttt{READ} signal just before any \texttt{SET} signal.

%We use \texttt{Bn} to denote the \texttt{n}-th binary register, (where we start counting at \texttt{n = 0}) just like we use \texttt{Un} to denote the \texttt{n}-th sliding block register, then the two of the input lanes of this register correspond to APGsembly actions that let us reposition the register's read head:\smallskip
%
%\begin{itemize}
%	\item \texttt{DEC Bn}, which decreases the position of the read head (if possible). If the read head was already at the least significant bit then it does not move and \texttt{Z} is returned. Otherwise, the read head moves and \texttt{NZ} is returned.\footnote{This action is completely analogous to the \texttt{TDEC} action for sliding block registers (and indeed, the binary register from Figure~\ref{fig:binary_register} has a sliding block register baked into it for exactly this reason). In particular, the \texttt{Z}/\texttt{NZ} return value is determined by the position of the read head \emph{before} decrementing it.}\smallskip
%	
%	\item \texttt{INC Bn}, which increases the position of the read head. If the bit at the new position of the read head has never been accessed (i.e., we have never \texttt{INC}ed this far before), \texttt{Z} is returned. Otherwise, \texttt{NZ} is returned.\smallskip
%\end{itemize}

% make sure to mention that read head starts at the least significant bit and there is no bound how many bits there are

% This is a good place to talk about why we need to store number of bits in binary register, and INC operation's return value can help us keep track of this

% NJ TODO: example APG code for storing a particular value in memory by working one bit at a time, to illustrate T read and write operations.
% here-ish: talk about difference between SBR being unary and T being binary


%% WHENEVER WE FIRST SEE NOP (maybe right after binary ruler?):
%If the NOP commands are added back in---e.g., ``OUTPUT "U0<=U1", NOP''---then the program will still halt, just in a slightly different way. The finite-state machine would reach the HALT state with no problem; NOP sends back a Z return value, so the HALT Z lane would be activated. The standard next step would be to send gliders to perform all the actions specified for that state/return value combination.  But that list is blank for HALT Z, so no gliders will be sent, and the pattern will stabilize at that point.
%
%Recent versions of APGsembly also allow two pieces of shortcut syntax which allow us to avoid the code duplication that we saw in the previous listing.\footnote{As a somewhat technical note, current APGsembly compilers include a simple preprocessing stage. The preprocessor removes blank lines and comments, and also converts any code that contains these two syntactical shortcuts to the older syntax above, before the compilation process starts.}\smallskip
%
%\begin{enumerate}
%	\item[2)] \texttt{ZZ} syntax: if there is no way that a given state can ever receive an \texttt{NZ} return value---e.g., if the given state is the program's initial state, or if the only way of reaching that state is via a jump after a \texttt{NOP} instruction---then a \texttt{ZZ} means that no \texttt{NZ} branch is needed. While a \texttt{*} could also be used in this case, the \texttt{ZZ} value is useful since then the APGsembly compiler may leave out the \texttt{NZ} mechanisms entirely, resulting in a slightly smaller Life pattern.\smallskip
%\end{enumerate}


% TODO summarize here: INC never has a return value, TDEC for all 3 components checks return value and THEN decs
...
Fortunately, since we are just increasing the value of the register by \texttt{1}, the carry bits are not difficult to take care of: we look for the least significant \texttt{0} bit and set it to \texttt{1}, and we set all bits that are less significant than it (which are currently equal to \texttt{1}) to \texttt{0}. Slightly more explicitly, we can increase the value of a binary register by \texttt{1} via the following procedure:\smallskip
...
This method is implemented in APGsembly~\ref{alg:apgsembly_binary_ruler}, which counts in binary by repeatedly adding \texttt{1} to the value of a binary register \texttt{B0}. This APGsembly also contains one new action that we have not yet seen: \texttt{NOP}.\index{NOP} This is an old standard programming abbreviation, short for ``No OPeration''; it tells the computer not to change anything right now, but emit a \texttt{Z} return value anyway. It is typically used in conjunction with other actions that don't have a return value (\texttt{SET B0} and \texttt{INC B0} in this case), since exactly one action in every state's list of actions must return a value of either \texttt{Z} or \texttt{NZ}.\footnote{It may be tempting to merge multiple lines of APGsembly~\ref{alg:apgsembly_binary_ruler} together so as to get rid of the \texttt{NOP} actions. For example, we might want to remove the \texttt{LSB1} state altogether and replace the \texttt{CHECK1;Z} line with the code \texttt{CHECK1; Z; LSB2; SET~B0,~TDEC~B0}. The problem with this is that it is not a good idea to perform two actions on the same register (\texttt{B0}) in the same clock tick, as it is not clear which one will be performed first (or even worse, they might interfere with each other and cause the register to self-destruct).}
...
%% NJ TODO: Is this actually O(log(t))? Might be even slower than that since operations on far-away bits take longer to complete?
...
While addition and subtraction of values that are stored in unary (sliding block) registers is reasonably straightforward (refer back to Section~\ref{sec:compiled_add_two_registers}), these operations are more complicated for binary registers. Even just adding \texttt{1} to the value of a binary register makes use of four lines of code (the first four lines from APGsembly~\ref{alg:apgsembly_binary_ruler}). The reason for the increase in complexity in this case is that when we add or subtract two binary numbers, we have to keep track of digits that are carried from one bit to the next, possibly many times in a row.

In order to make these operations easier to perform, we introduce additional components that are custom-made for storing these carry digits. For example, the \texttt{ADD} and \texttt{SUB} components displayed in Figures~\ref{fig:add_component} and~\ref{fig:sub_component}, respectively, perform bitwise operations to assist in the addition and subtraction of binary registers. The \texttt{ADD} component performs the addition of two bits, outputting the least significant bit of the addition and storing the carry bit in its own internal memory, and the \texttt{SUB} component similarly performs the subtraction of two bits while keeping track of the borrow bit.

% TODO in this section somewhere: idea that we need to store number of bits allocated to binary register when doing arithmetic on it, since we work LSB to MSB and have no way of otherwise knowing when we have reached the MSB

% Footnote never need more than one ADD/SUB/MUL, unlike other registers
% Footnote that like binary registers, these could be made smaller nowadays with modern tech if desired.

% TODO: Exercise -- why/how do these differ? How do they store a memory bit?
...
% TODO: Also note that appending B0 clears memory of ADD, which is important
% NJ TODO: preload values into this APGsembly and also later MUL apgsembly
...
% TODO: Mention earlier than we use subscript 2 to denote binary (and be more consistent about this)
...
% Emphasize idea that we should just use unary registers for small values. Only binary for large values.
...
\begin{itemize}
	...
	%	
	%	\item It is \emph{fast}. Our Life computer adds a lot of computational overhead to the algorithm that it implements, so if we want to actually see our calculator print out a decent number of digits of $\pi$, we should choose an algorithm that works very quickly. As part of this, we would like the integers used in our computation to be reasonably small, since our addition and multiplication operations take longer to compute on larger inputs.\smallskip
\end{itemize}
...
% TODO: explain a bit where these matrices come from
...
% NJ TODO: Be more specific about the last line in this paragraph: where did we see how to add/multiply binary registers?
...
The procedure outlined above results in the image displayed in Figure~\ref{fig:koch_algorithm}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{universal_computation/koch_algorithm.pdf}
	\caption{The leftmost portion of the image that results from starting at the bottom-left corner (circled in \bgbox{redback}{red}) and printing pixels according to a path-following procedure based on counting in base~$4$. These bulbous shapes are better and better approximations of one half of an 8-pointed version of the Koch snowflake fractal.}\label{fig:koch_algorithm}
\end{figure}
...
\begin{apgsembly}
	...
	\begin{minipage}[t]{.49\textwidth}
		...
		\end{algorithmic}
	\end{minipage}
	\caption{APGsembly code the Koch snowflake printer.}\label{alg:apgsembly_koch}
\end{apgsembly}

% Make sure to credit Simkin. This is one of the few things from this chapter that is not Adam's

% Mention that B2D can print other letters/symbols if desired

% Need to mention that B2D requires a slower clock (2^24?), and this is also why it needs such a big space between logic unit and thing it prints
...
% NJ TODO: Cite Adam's chapter "Universal Computation and Construction in GoL Cellular Automata" in Adamatzky's book
% NJ TODO MUCH EARLIER: Link to download location for APG compiler etc.
...
% EXERCISE TODO: Copy binary register into another one?
...
% NJ TODO: Modify statements like "first 12 lines of pi APGsembly" in light of now using the #REGISTERS code
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{problem}\label{exer:division_by_subtraction}
	...
	\begin{enumerate}[label=\bf\color{ocre}(\alph*)]
		...
		\item Use the APGsembly compiler that is linked at \httpsurl{conwaylife.com/wiki/APGsembly} to compile a Life pattern that implements your APGsembly code from part~(b).
	\end{enumerate}
\end{problem}
...
\begin{problem}\label{exer:universal_computation_b2d_why_far}
	...
	\noindent [Hint: You can move the sliding block at the top-left corner a bit closer to the logical circuitry without breaking anything, but something goes wrong if you move it, for example, $5,000$ cells closer.]
\end{problem}
...
\begin{problem}\label{exer:universal_computation_print_prime}\index{prime number}
...
\begin{problem}\label{exer:universal_computation_print_fibonacci}\index{Fibonacci number}
