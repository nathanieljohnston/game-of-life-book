...
% ?? Figure~\ref{fig:binary_register} here.
% Waiting for the B register to be completed.
...
\begin{itemize}
	\item An \texttt{INC}\index{INC} action moves the pointer block (called the \emph{read head}\index{read head} of the register) to the next bit location, one step farther away.\smallskip
...
% NJ question for Dave: Is the #REGISTERS syntax for the B0 register in the above footnote correct?
...
% ?? Figure~\ref{fig:binary_ruler} here.
% Waiting for the B register to be completed.
...
%But each use of ADD or SUB is made up of two calls to it. So there's another sense more in line with individual lines of APGsembly, that says that ADD and SUB temporarily store two separate bits of information. Between the A{n} call (if any) and the B{n} call, ADD and SUB are tracking the carry bit and the A input value separately. Those values couldn't be combined and stored in a single one-bit memory cell, because an internal A0+{0 carry bit} results in a different internal state from A1+ {1 carry bit}, even though they both have a zero output. Between A{n} and B{n} the component can be in one of three possible states.
...
Now that we know how to add and subtract binary components, we can also multiply and divide them via the same tricks that we used for sliding block registers in Section~\ref{sec:multiply_two_registers} and Exercise~\ref{exer:division_by_subtraction}. That is, we simply add or subtract repeatedly. However, doing so can be quite time-consuming when the registers store large values, so we now introduce one additional \index{MUL component} custom binary register arithmetic component---one that helps us multiply a binary register by $10$.\footnote{The reason for us making $10$ easier to multiply by (as opposed to any other fixed integer) is that multiplication by $10$ is a common operation when working with decimal numbers. In particular, we will make use of this multiplication-by-$10$ component to help us extract digits in the upcoming $\pi$~calculator in Section~\ref{sec:pi_calc}.}
...
% Need to mention that B2D requires a slower clock (2^24?). this is also why it needs such a big space between logic unit and thing it prints
