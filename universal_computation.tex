...
% TODO: Double-check pi APGsembly. It preloads B0 to be 2, for example. I assume this means pre-loading the position of the read head, not the value in memory? Compiled pattern doesn't seem to do either yet, so I'll have to manually adjust.
...
\begin{figure}[!htb]
	...
	\caption{A $\pi$ calculator that uses a computer (highlighted in \bgbox{aquaback}{aqua}) based on the APGsembly code from Appendix~\ref{sec:appendix_apg}. Shown here at generation $8 \times 10^{13}$, at which point it has printed the first $13$ digits of $\pi$: $3.1415926535897$.}\label{fig:pi_calc}
\end{figure}
...
Paul Rendell developed the first such device in April 2000 \cite{RenBook}---a Turing machine based on period~30 circuitry (stable circuitry was not nearly as well-developed at the time). Then developed...% two other follow-up turing machines.

Paul Chapman's Minsky Register Machines in 2002, Adam~P.~Goucher's Spartan universal computer-constructor in 2009 \cite{Gou10} (which was the basis for almost all of the components that we investigated in this chapter), and Nicolas Loizeau's 8-bit programmable computer in 2016.
...
About one week prior to building his $\pi$ calculator, Goucher built a similar calculator for the mathematical constant $\varphi = (1+\sqrt{5})/2 = 1.61803\ldots$. While the techniques of Section~\ref{sec:pi_calc_other} are general enough to build a $\varphi$ calculator, the original one used a much simpler algorithm that caused it to run significantly faster---it could compute the first $n$ digits of $\varphi$ in $\Theta(n^3)$ generations, where as the $\pi$~calculator requires $\Theta(n^6)$ generations.
...
% History: http://www.igblan.free-online.co.uk/igblan/ca/ (mostly about Paul Chapman's MRM 2002)
