...
This all leads to a very natural question: why those rules? Why not have a dead cell come to life only in the case that it has exactly 4~live neighbors? Why not have a live cell stay alive if it has exactly 1, 2, 4, or 7 live neighbors? Indeed, there are $2^{18} = 262{\thousep}144$ distinct \emph{Life-like cellular automata}:\index{Life-like} rules that can be applied to a 2D square grid of alive and dead cells that simply depend on the numbers of live and dead neighbors that lead to a cell staying alive or coming to life. However, the following three properties make Life special (but by no means unique):\smallskip
...
Nonetheless, other birth and death rules are sometimes studied as well, and for brevity they are typically described using a \emph{rulestring}\index{rulestring}\index{Bx/Sy|see {rulestring}} of the form Bx/Sy, where we replace ``x'' by all numbers of live neighbors that lead to the \textbf{b}irth of a dead cell, and we replace ``y'' by all numbers of live neighbors that lead to the \textbf{s}urvival of a live cell. For example, the Game of Life is described by the rulestring B3/S23. We will discuss other rules much later, in Chapter~\ref{chp:0e0p}, when we develop machinery that lets us run other cellular automata within Life.
...
Sometimes, it is possible to ``fix'' unstable evolutionary patterns by tweaking them in some simple way, thus creating interesting objects that do not often appear naturally by themselves. Perhaps the most useful example of such a pattern is the \emph{queen bee}, which is a commonly-occurring object\footnote{For example, it appears in generation~$1{\thousep}185$ of the soup in Figure~\ref{fig:random_2}.} that reflects itself after $15$~generations, but leaves behind a beehive in the process. It follows that after 30~generations, the queen bee is back where it started, but with an additional beehive on either side of it (see Figure~\ref{fig:queen_bee}). Shortly after 30~generations, the queen bee collides with the first beehive that it created, resulting in its self-destruction.
...
\begin{figure}[!htb]
	...
	\caption{An R-pentomino takes $1{\thousep}103$ generations to stabilize---much longer than any other polyomino with 5 or fewer cells. Along the way, it produces numerous other important patterns such as the \emph{B-heptaplet} highlighted in \bgbox{aquaback}{aqua} and the \emph{Herschel} highlighted in \bgbox{magentaback}{magenta}.}\label{fig:r_pentomino}
\end{figure}
	...
	\begin{figure}[!htb]
		...
		\end{minipage}\hfill\begin{minipage}[b]{0.32\textwidth}
			\centering\gridbox{1pt}{\begin{tikzpicture}[scale=0.5, every node/.style={transform shape}]%
				\node[inner sep=0pt,anchor=south west] (glider_loop) at (0.5,0.5) {\includegraphics[width=6cm]{early_life/goe_1_row_proof.png}};
...
\begin{figure}[!htb]
	...
	\begin{minipage}[b]{0.48\textwidth}
		\centering\includegraphics[width=0.75\textwidth]{early_life/sci_am_1970.png}
	...
	\end{minipage}\hfill
	\begin{minipage}[b]{0.48\textwidth}
		\centering\includegraphics[width=0.75\textwidth]{early_life/lifeline_vol1.png}
