...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Waterbear}\label{sec:waterbear}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Stuff.

Remarkably, this is not a problem---the exact same reactions that we used in Section~\ref{sec:caterpillar_helices} to create orthogonal helices can in fact be used to create \emph{oblique} helices that travel at the same speed and angle as any spaceship.\footnote{We know from Exercise~\ref{exer:general_speed_limit} that a spaceship that displaces itself by $(x,y)$ over the course of its period must have speed no larger than $(x,y)c/(2x+2y)$.} For example, Figure~?? illustrates how these reactions can be used to make a helix that travels at... (SOME OBLIQUE SPEED).

% introduce backward-firing component, phrase theorem in terms of it instead. Leave forward version of oblique to exercises.

\begin{theorem}[Universality of Oblique Helices]\label{thm:universality_oblique_helices}
	For any integers $x,y > 0$, the three reactions from Figure~\ref{fig:helix_operations} can be used to create an oblique helix, which fires gliders forward on one side, with any rational speed that is no faster than $(x,y)c/(2x + 2y)$. In particular, such helices exist that travel at the same speed and direction as any oblique spaceship.
\end{theorem}

\begin{proof}
	We proceed mostly as in the proof of the corresponding result for orthogonal helices (Theorem~\ref{thm:universality_orthogonal_helices})---the main difference is that instead of using $m$ of the glider-shifting flotillae in each direction of the helix, we use $k$ of them in one direction and $m$ of them in the other (so, for example, $k = ??$ and $m = ??$ in the oblique helix from Figure~??). This helix travels at a speed of
	\[
		\frac{(10k + 10m + 20, 13k - 13m)c}{20k + 20m + n + 101}.
	\]
	% NEED TO FIX SOEMTHING HERE RE: POSITIVITY
	To create such a helix with speed $(x,y)c/q$, where $x$, $y$, and $q$ are arbitrary positive integers with $x/q, y/q < 1/2$,\footnote{This is actually a stronger result than in the statement of the theorem. These helices can travel diagonally at a speed of $c/2$, for example, despite the $c/4$ diagonal spaceship speed limit. The reason for this apparent discrepancy is that the glider is not travelling through empty space in one of the directions---it is burning through xWSSes.} we can simply choose
	\begin{align}\label{eq:oblique_helix_parameters}
		k = 10y - 13x + 1, \qquad m = 10y + 13x - 1, \qquad \text{and} \qquad n = 260r - 520x - 61.
	\end{align}
	The fact that $m$ is a non-negative integer is clear, and $n$ being a non-negative integer follows from the fact that $p/q < 1/2$, so $q - 2p \geq 1$. Some mildly ugly algebra also reveals that
	\begin{align*}
		\frac{20m + 20}{40m + n + 101} & = \frac{20(4p - 1) + 20}{40(4p - 1) + (80(q - 2p) - 61) + 101} \\
		& = \frac{80p - 20 + 20}{160p - 40 + 80q - 160p - 61 + 101} = \frac{80p}{80q} = \frac{p}{q},
	\end{align*}
...
% Note that we can actually REACH oblique speed limit, whereas we can only approach orthogonal speed limit. This is because we can burn through non-empty space in the oblique direction (we can actually EXCEED the speed limit in that direction)
...
% Exercise: make helix with (speed)
% Exercise: remove (something) from helix, what is new speed?
