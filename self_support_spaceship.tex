...
\begin{proof}
	...
	\[
		\frac{(100 + 2n)c}{2\big((400 + 2n + 2m) + (100 + 2n)\big)} = \frac{(50 + n)c}{500 + 4n + 2m}.\footnote{If we calculated this speed based on the loaf-\emph{pulling} flotillae instead of the loaf-\emph{pushing} ones, we would get the same answer: $(100 + 2n)c/\big(2((600 + 6n + 2m) - (100 + 2n))\big) = (50 + n)c/(500 + 4n + 2m)$.}
	\]
	...
	as claimed.
\end{proof}
% Note not optimal, caterloopillar we showed above overclocks its glider to make flotillae shorter

% EXERCISE: Adjust (by hand) caterloopillar by hand to be faster/slower than ??.
% Footnote that certain speeds not attainable by scripts, but easily reached by hand

% Exercise: we saw glider-reflecting flotillae in Chap 4. Why do they not apply here?
...
% As speed increases, spacing between flotillae doesn't just increase, but number of steps in slow salvo synth also increases

% backward 1274 separation. Exercise: compute backward sep from forward and displacement?
% script does not construct all speeds, but that is OK since we can adjust by hand
% TODO: Come back to doppler effect in Chap 11 when discussing gemini and other speed limits?
% Exercise: Use script to make a c/21 caterloopillar. Follow-up exercise: adjust by hand to make a c/22 caterloopillar (just separate components a bit or move one end farther away?)
