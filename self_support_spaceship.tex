...
When measuring size according to number of live cells, these spaceships are roughly $16$ and $3$ times as large as the silverfish, respectively. The reason that the silverfish could be so much smaller that it uses a new front end (which was found by Chris Cain just 16 days after he completed the centipede) that is supported by just two heavyweight spaceships on each side instead of six.

There are numerous other reactions that could potentially be used to build self-supporting spaceships other than the ones that we have seen.

% https://www.conwaylife.com/wiki/Crawler
...
% lots of exercises here: make helix that matches speed of (reaction given above). Find trail based on those reactions that produces forward/backward glide (by placing 2 next to each other), etc.
...
% Gabriel's caterpillar page has another potential reaction (it's the (13,1)c/31 from above). Any others? Maybe list a bunch.
