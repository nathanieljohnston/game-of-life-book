...
% lots of exercises here: Find trail based on those reactions that produces forward/backward glide (by placing 2 next to each other), etc.

% Talk about how caterloopillar blurs the line between self-supporting and universal construction? Or at the start of next chapter instead?

% EXERCISE: put (13,1)c/31 on a block track too: https://conwaylife.com/forums/viewtopic.php?f=2&t=1065#p7750  This is less useful since the B-hep still makes an output glider, not block (in contrast with (34,7)c/156 crawler exercise)
...
\end{problem}
% SOLUTION: 3-xWSS component reflects near its front and can thus be used to make helices that are much faster (without using the c/2 helix component)). To make the 2-xWSS components fast, we need to add lots of other xWSSes to speed it up.
...
\end{problem}
% Solution: It creates a seed for back half of caterloop (to construct forward-moving ships).
...
\begin{problem}\label{exer:13_1c_31_helix_large}
	...
	\begin{enumerate}[label=\bf\color{ocre}(\alph*)]
		...
		% SOLUTION: k + m >= ceil(693/50) = 14, so at least 28 + 2 + 3 = 33 xWSSes. This is valid because the 10/20 mover is the fastest component, so helix is made fastest by repeating it (not repeating ends, for example).
		
		\item Show that any such helix consisting only of those three components must actually contain at least 35 xWSSes.
		% SOLUTION: Suppose that k + m = 14, as suggested in part (a). Then looking at speed in one direction gives (140 + y + 20)/(20*8 + 20*6 + x + 4*y + 101) = 13/31, which has no positive integer solutions (it is equivalent to 13x + 21y = 7).
		...
		% k = 14
		% m = 14
		% a (free glider cells in k direction): 25
		% b (same in m direction): 0
		% n = 14
	\end{enumerate}
\end{problem}
