...
\begin{table}[!htb]
	\begin{center}		
		\begin{tabular}{r c l}
			...
			all speeds $< c/4$ & diagonal & ``Demonoid'' engineered spaceships\index{Demonoid}\protect\footnotemark \\
			\rowcolor{gray!20} $(2,1)c/6$ & slope $2$ & Sir Robin \\
			$(6,3)c/2621440$ & slope $2$ & ``half-baked knightship''\index{half-baked knightship} \\
			\rowcolor{gray!20} all speeds $< (1,1)c/580$ & all slopes & ``Geminoid''\index{Geminoid} engineered spaceships\protect\footnotemark \\\bottomrule
		\end{tabular}
...
\stepcounter{footnote}\footnotetext{All rational diagonal speeds slower than $c/4$ are attainable using a ``Demonoid'' construction, which we will discuss in Section~\ref{sec:fast_demonoid}. There is even a computer script that quickly builds such a spaceship of any desired speed.}
\stepcounter{footnote}\footnotetext{While essentially any direction and sufficiently slow speed can be attained by a Geminoid spaceship, most speeds that have been explicitly constructed in this way are in the ballpark of $c/6500$.}
...
\begin{problem}\label{exer:general_speed_limit}
	...
	\begin{enumerate}[label=\bf\color{ocre}(\alph*)]
		...
		% c/4 diagonal, c/2 orth, c/4 orth. TODO: Make sure text has example of c/4 orth somewhere.
	\end{enumerate}
\end{problem}
