...
\begin{figure}[!htb]
	...
	\begin{tabular}{@{}ccc@{}}
		...
		\end{subfigure}	
	\end{tabular}
	\caption{Two guns formed by placing two copies of Tanner's p$46$ next to each other so that their sparks interact. The glider gun from (a) can be used as an inline inverter, as in (b), where the incoming glider (highlighted in \bgbox{magentaback}{magenta}) prevents a glider from being fired from the gun at that point in the stream. The gun in (c) is the smallest known MWSS gun.}
...
\begin{figure}[!htb]
	...
	\caption{A small period~$92$ Heisenburp that uses two twin bees shuttles (highlighted in \bgbox{yellowback2}{yellow}) to copy a glider (highlighted in \bgbox{aquaback}{aqua}) without affecting it. The first $46$ generations are used to create a block (highlighted in \bgbox{magentaback}{magenta}) from the glider, and the next $46$ generations are used to turn the block into another glider.}\label{fig:p46_natural_heisenburp}
\end{figure}
...
% Exercise: Make p not-46 version of this Heisenburp?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{problem}\label{exer:inline_inverter_gun_high_period} \probdiff{1}
		...
		\end{enumerate}
	\end{problem}
% SOLUTION: Some of these are given in primer.
	...
	\begin{problem}\label{exer:make_inline_inverter_with_weird_period} \probdiff{3}
		Use the reaction from Figure~\ref{fig:stream_inverter}, together with reflectors and eaters as necessary, to create an inline inverter for a glider stream with...\smallskip
	...
	\begin{problem}\label{exer:twin_primer} \probdiff{3}
		The collision of a glider with an LWSS shown below is called a \emph{filter}\index{filter}. It results in a single block that destroys the next incoming glider, letting the next incoming LWSS pass by unharmed, thus doubling the period of the LWSS stream.\smallskip
	...
	\begin{problem}\label{exer:p30_ticker_tape_gun} \probdiff{3}
		Rebuild the ticker tape gun from Figure~\ref{fig:ticker_tape_gun} so that it uses p$30$ circuitry instead of p$46$.
	\end{problem}
	...
	\begin{problem}\label{exer:ticker_tape_gun_memory_alter} \probdiff{1}
	...
	\begin{problem}\label{exer:twin_bees_large_spark} \probdiff{3}
