...
\begin{itemize}
	...
	% NJ is here. Don't forget to add snark/reflector timing exercise from earlier.
	% Also mention easier any-period inversion method first!
	% Exercise: Find repeat time and explain why it's not just 60, for example. (it is 355)
	% Recolor stuff above. Modify exercise that calls the reflectors magenta.
...
% SOLUTION: Some of these are given in primer.
	...
	\begin{problem}\label{exer:twin_bees_large_spark} \probdiff{3}
		...
		\begin{enumerate}[label=\bf\color{ocre}(\alph*)]
			...
			\item Collide the spark from this object with the large-spark version of the twin bees shuttle from Figure~\ref{fig:twin_bees_reflect} so as to create a double-barreled glider gun\index{double-barreled gun} (i.e., a gun that emits two side-by-side gliders per period).\footnote{This double-barreled gun was originally found by Dieter Leithner.}
		\end{enumerate}
	\end{problem}
...
%59b2o5b2o43b2o5b2o!
	...
	\begin{problem}\label{exer:bumper_bouncer_wrong_color} \probdiff{1}
	...
	\begin{problem}\label{exer:p11_bumper_not_useful} \probdiff{2}
	...
	\end{problem}
	% SOLUTION: Minimum repeat time is 44 generations. For p44, 55, ..., 110, can use smaller p4, p5, p6, p7, p8, p9, and p5 bumpers instead.
	...
	\begin{problem}\label{exer:p60_regulator_mwss_emulator} \probdiff{2}
	...
	\end{problem}
	% SOLUTION: Filters the p30 LWSS stream to be p60
	...
	\begin{problemstar}\label{exer:prng_gun}
		...
		\begin{enumerate}[label=\bf\color{ocre}(\alph*)]
			...
			\item \probdiff{3} Determine the period of the gun constructed in part~(a). [Hint: Computer software may be useful here---it is massive.]
