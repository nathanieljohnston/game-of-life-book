...
The other three child metacells extract their new states from the lookup table similarly:\smallskip

% TODO: Finish this.
\begin{itemize}
	\item \textbf{NE child}:\smallskip% say how many gliders destroyed, when Herschel produces escaping glider, and when extraction started by hitting eater 1. Repeat for other 2 children.
\end{itemize}

% Need 1 more paragraph or so, saying that after these gliders are extracted, they are redirected towards the blocks for storage. Figure is probably a good idea. No need to streamlife -- this happens just a couple thousand generations later.
