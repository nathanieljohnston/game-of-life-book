...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Shell and Switching Circuitry}\label{sec:0e0p_structure_shell}\index{shell}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The rotationally symmetric shell that makes up the outermost edges of the 0E0P metacell is perhaps the simplest one of its three pieces. Each of its four sides has four input lanes (in green and grey in Figure~\ref{fig:0e0p_schematic}). However, only one of these four input lanes is ever actually used (in green---a different one on each side), as they connect to the four output lanes coming from the kernel of neighboring metacells (in solid magenta in Figure~\ref{fig:0e0p_schematic}).\footnote{This design is similar to that of a circuit on a motherboard---you can think of each side of the 0E0P metacell as having 8 ``pins'' (4 inputs pins and 4 output pins) that connect to the 8 pins of neighboring metacells.} The remaining input lanes (in grey) are unused, and only exist to enforce the shell's fourfold rotational symmetry.

As the metacell is being constructed by its neighbors, these input lanes read in a copy of the 0E0P's rule table and single-channel construction recipe from them. That (extremely long) sequence of gliders revolves clockwise all the way around the shell along its spiral arm, until it is finally injected into the nucleus at the 0E0P's center.
% QUESTION: how does it only read in one copy if, say, 3 neighbors are alive? Likely some kind of switching mechanism, so a figure might be warranted
% TODO: should probably have a (sub?)section devoted to switching circuitry. Or at least a paragraph and a figure? Things like the block being removed in a syringe so that it now just eats things
