...
\begin{figure}[!htb]
	...
	\caption{The subroutine glider loop has been used once at \texttt{LW} to construct the first set of Snarks along the nucleus's northwest wall (the last elbow operation from that loop is about to pull the elbow block circled in \bgbox{redback}{red} by 14fd). The glider from Figure~\ref{fig:0e0p_timeline_397258593} is between \texttt{CE} and \texttt{SE}, on its way to \texttt{S}.}
...
% MORE: https://www.conwaylife.com/forums/viewtopic.php?f=11&t=3089&hilit=MAP&start=50#p98128
	...
	\begin{problem}\label{exer:0e0p_single_cell_rro_rule}
		The 0E0P metacell from Figures~\ref{fig:0e0p_timeline_0}--?? is emulating the isotropic rule
		\begin{align*}
