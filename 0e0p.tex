...
Construction of the child's symmetric shell continues in several stages. Often a stage begins with the construction of a Cordership, followed by a slow salvo to shoot down the Cordership once it has traveled the right distance and turn it into a new target ``hand'' block at the correct location, and then followed by a slow salvo recipe to build whatever is needed at that location. There are many visible gaps in the 0E0P's single-channel recipe, and most of them correspond to the flight time of a Cordership, while the recipe waits to shoot it down after it has traveled the right distance.
...
That single extra glider that was shot sideways by the temporary elbow will be used shortly to trigger construction of the southeast child metacell's kernel.
...
\begin{figure}[!htb]
	...
	\caption{The child's kernel has been constructed. A glider goes through a temporary Snark at its south corner that leads it to \texttt{CN}, which will trigger construction of the nucleus's northwest wall.}
