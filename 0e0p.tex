...
In the actual 0E0P, the clock gun... % lead in to figure here, but have actual figure later
...
% Rule stored in sequence of up to 8 blocks at SW corner of logic component
...
\begin{itemize}
	\item Cycle 0: The metacell constructs its southeast neighbor.\smallskip
	
	\item Cycle 1: It duplicates the contents of its nucleus into that of the southeast neighbor.\smallskip
	
	\item Cycle 2: It constructs its northeast neighbor.\smallskip
	
	\item Cycle 3: It duplicates the contents of its nucleus into that of the northeast neighbor.\smallskip
	
	\item Cycles 4--7: Repeat cycles 0--3 for the northwest and then southwest neighbors.\smallskip
	
	\item Cycles 8--12: Do nothing (just spend some time ``looking like a cell'').\smallskip
	
	\item Cycle 13: Empty the single-channel glider stream out of the metacell's nucleus.\smallskip
	
	\item Cycle 14: The parent metacell sends its state to all four of its children.\smallskip
	
	\item Cycle 15: The parent metacell self-destructs while the child metacells compute their states.\smallskip
	
	\item Cycles 16--63: Child metacells either self-destruct (if their state is $0$) or do nothing (if their state is non-zero, they just spend time ``looking like a cell'').\smallskip
\end{itemize}
...
\begin{figure}[!htb]
	...
	\caption{The clock gun.}
	\label{fig:0e0p_clock_gun_full}
\end{figure}

% Maybe a figure showing that 2^29 output lane in the south control clock, labelling which cycles they correspond to? Eater 1 in there first while children wait, then is destroyed when state information arrives (right?), first output boat initializes state-reading clock, next boat does early death if computed state is 0 (by destroying the next slow-salvo block), that block absorbs 48 gliders (i.e., cycles), then 2 for SE child, 2 for NE child, etc, then empty out nucleus signal, then state-sending signal, then self-destruct signal
...
Stuff.
% Next: 1040192000 (should this actually be 1040187392 + 2^25?)

% Be careful with this figure -- timestamp stays the same, but figure itself needs to change when adding 2^25 to timestamp numbers
\begin{figure}[!htb]
	\centering
