...
% To convert a timestamp from here to rrocell_backup: t at start of RRO_file = 262144*RRO_NUM - 26375797384
% Short version: t = 0 corresponds to rrocell_100615 at generation 178824
% To get generation t, look in rrocell_X, where X = 100615 + floor((t+83320)/262144) at generation (t+83320) mod 262144
% The above formula might be off by 95502?

% For t = 7482638336, use generation 105727352 of rrocell_backup128756
...
%%%%%%%%%%%%%%%%%%%%%%
\subsection{Generations $\mathbf{0.76 \times 2^{29}}$ to $\mathbf{1.94 \times 2^{29}}$: Nucleus Fills While Being Constructed}\label{sec:0e0p_timeline_nucleus_fill}
%%%%%%%%%%%%%%%%%%%%%%
...
For example, the \emph{next} three semi-Snarks are destroyed just $2^{26}$ generations later, by the Herschel that appears at $t = 1{\thousep}002{\thousep}397{\thousep}952 \approx 1.8671 \times 2^{29}$, and the clock gun vanishes completely at $t = ??$.
% Argh these timestamps based on the RRO files might be off by 2 generations?? need to double-check.

At $t = 1020357437 \approx ??$, a boat acting as a one-time turner sends an extra signal off to initiate a self-destruct chain reaction in the entire storage loop (since by this time it will be safely empty).

T = 1020519136: The NW storage loop cleanup job is complete -- the final spark is at T = 1020519135 at (13317,-131827) way off in the northwest. The clock gun finished cleaning itself up well before that long self-destruct chain reaction was complete.

Back in time slightly, at $t = 940{\thousep}814{\thousep}085 \approx 1.7524 \times 2^{29}$, the final glider in the single-channel recipe is about to enter the child's nucleus at the location just northwest of \texttt{LS} (see Figure~\ref{fig:0e0p_timeline_940814085}). Once this insertion is complete, the child's nucleus will be fully populated with a copy of the parent's DNA.
...
* NJ calls this T = 1040180152: The first glider in the child metacell's recipe finds its way through the long back-and-forth reflector chains in the nucleus, and makes its way back to a key splitter at (199708,196142). The first glider in this recipe passing through this splitter is what signals the child's main south control clock to activate, exactly on the right schedule -- though the clock has no outputs until a later timestamp removes a key eater. The clock start-up happens via a set of one-time turners on an otherwise dead-end branch of the splitter, starting at T = 1577051499 with the boat at (199640,196165).
...
The second glider in the recipe also gets a copy sent elsewhere via one-time turner, the boat at T = 1577051631 at (199628,196174) -- this time, down to the far south corner, to turn off the incoming recipe from the parent, to prevent the second copy of the non-coding information sent from the parent's nucleus from attempting to overwrite the first copy already in the child nucleus (which would have disastrous consequences).

The full replication cycle won't get started again for several billion ticks yet -- several communication and decision stages still have to be added to this list! However, at this point the child 0E0P metacell has reached a stable phase, where it will remain with (at most) minor internal adjustments through several full cycles of the memory tape through the nucleus.
