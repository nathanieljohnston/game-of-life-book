...
\begin{figure}[!htb]
	\centering
	\embedlink{state_lookup_clock_gun}{\includegraphics[width=0.8\textwidth]{0e0p/state_lookup_clock_gun.pdf}}
...
\clearpage% Here to fix a dangling line. Remove if layout changes and this causes problems.
...
% TODO: Put the correct figure here, once the timestamp has finished computing. This is just here for spacing reasons.
\begin{figure}[!htb]
	\centering
	\patternlink{0e0p_timeline_8124366848_small}{\includegraphics[width=1\textwidth]{0e0p/0e0p_timeline_8124366848.pdf}}
...
\begin{figure}[!htb]
	...
	\caption{The \textbf{OTCA metapixel}, with orthogonal rows of lightweight spaceships filling up its central region, making it look ``alive''. By adjusting which eaters are present on its left edge, it can be made to emulate any Life-like cellular automaton. The eaters displayed in \bgbox{redback}{light red} are not actually present---they are just displayed so as to show their potential locations---so this metacell is emulating the rule B3/S23 (i.e., Life).}
...
The next notable metacell to be constructed was the \textbf{p1 megacell},\index{p1 megacell}\index{megacell|see {metacell}} by Adam P.~Goucher in 2008 (see Figure~\ref{fig:p1_megacell}). This metacell was, again, larger and slower than the metacells that came before it, with a bounding box of $2^{15} \times 2^{15}$ and a period of $2^{24} = 16{\thousep}777{\thousep}216$. The new features this time that warranted the extra size and delay were twofold:\smallskip

\begin{figure}[!bht]
	\centering
	\embedlink{p1_megacell}{\includegraphics[width=1\textwidth]{0e0p/p1_megacell.pdf}}
	\caption{The \textbf{p1~megacell}, stuff. PLACEHOLDER.}
...
% EXERCISE: in which chunk of 0E0P lookup table would you find the state for a child with parents (describe 4 parent states)
% EXERCISE: modify rule table of p1 megacell to emulate (some rule).
