...
To compute its state from those of its parents, a child metacell uses a lookup table: a collection of $8^4 - 1 = 4{\thousep}095$ sequences of $0$--$7$ gliders each, specifying what the new state should be given any possible combination of not-all-zero parent states. This lookup table is stored in the nucleus, and the child metacell uses the block absences to pluck out the correct piece of it.

Specifically, the child metacell uses a \textbf{state-lookup clock gun},\index{state-lookup clock gun} as illustrated in Figure~\ref{fig:state_lookup_clock_gun}, to ...

% TODO: Go back to Figure 12.18 and highlight which part is the rule table and which part is the rest of the single-channel recipe.
...
% TODO: put more labels on wires on 0E0P schematic?
...
% FIGURE {fig:0e0p_timeline_8053072526}
