...
%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Next $\mathbf{?? \times 2^{29}}$ Generations: Nucleus Fills While Being Constructed}\label{sec:0e0p_timeline_nucleus_fill}
%%%%%%%%%%%%%%%%%%%%%%

At generation ??, the parent metacell's clock gun releases a glider that... switch to output lane instead of construction lane.

So now both loops are filled and are building the repetitive Snark chains that will hold the child 0E0P's copy of the full recipe. When construction is done there will be 246 copies of 8 Snarks in long chains along the NW and SE sides of the "nucleus". The sets of 8 Snarks and their associated self-destruct circuitry are not quite mirror-images of each other, so different recipes are needed for the NW and SE subroutine storage units.

This goes on for quite a while. The clock guns are period 256* $2^21$ = $2^29$. The glider storage loops are period 2097152, and they have to cycle 256 times each and then get shut down cleanly. The recipes in the subroutine storage units are actually for just two Snarks (plus their associated self-destruct circuitry). Splitters near the storage units duplicate the recipe several times, so that construction actually proceeds on four Snark pairs simultaneously. The unit of repetition that's constructed 256 times is made up of eight gliders, offset in pairs from each other so that the simultaneous constructions don't get in each other's way.

* T = 935289088: a Herschel is present in the NW clock's base p256 gun that will produce the glider that makes it through all of the twenty-one semi-Snarks in the loops below. That signal knocks out a key block in a syringe in the NW subroutine loop, so that all further recipe gliders in the loop will be harmlessly absorbed by the syringe's eater2 -- no more building will occur.

A side branch of that same signal also starts the gradual process of dismantling the gun. Every time three of the semi-Snarks are destroyed, the speed of the destruction process increases by a factor of 8 -- so, basically, nothing much happens for a long time, and then everything happens fairly quickly.

\begin{figure}[!htb]
	\centering
	
	\patternlink{0e0p_timeline_935289088_small}{\includegraphics[width=1\textwidth]{0e0p/0e0p_timeline_935289088.pdf}}
	\caption{A Herschel is present in the \texttt{CN} clock gun that will produce the glider that makes it through all of the twenty-one semi-Snarks in the loops below. That signal destroys a block in a syringe (circled in \bgbox{redback}{red}), and also destroys the three outermost semi-Snarks, making the clock gun $8$ times faster.}
